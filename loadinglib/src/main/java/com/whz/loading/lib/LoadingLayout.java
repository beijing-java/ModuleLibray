...
//        loadingPage.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        loadingImage = loadingPage.findViewById(R.id.loading_img);
        loadingImage.setBackgroundResource(R.drawable.loading);
        loadingImage.setWillNotDraw(true);
        loadingAd = (AnimationDrawable) loadingImage.getBackground();
        loadingPd = new Dialog(mContext, R.style.loaddialog);
        loadingPd.setContentView(loadingPage);
...
//                loadingPd.setContentView(loadingPage);
    ...
    public void stopAnim() {
        if (loadingAd != null && loadingAd.isRunning()) {
            loadingAd.stop();
        }
    }

    public void showLoading() {
        if (loadingPage.getVisibility() == VISIBLE && !loadingPd.isShowing()) {
            loadingAd.start();
            loadingPd.show();
        }
    }
    ...
    public static class Config {
        ...
        int emptyImgId = R.mipmap.receive_no_data_icon;
        ...
        int loadingLayoutId = R.layout.widget_loading2_page;
